...
Download_ocserv(){
	...
	tar -xJf ocserv-1.2.2.tar.xz && cd ocserv-1.2.2
	./configure
	make
	make install
	cd .. && cd ..
...
Service_ocserv(){
	if ! wget --no-check-certificate https://raw.githubusercontent.com/jackzhang-superman/ocserv/main/ocserv_debian -O /etc/init.d/ocserv; then
...
Installation_dependency(){
	...
	if [[ ${release} = "centos" ]]; then
		...
		if [[ $? = 0 ]]; then
			...
			wget --no-check-certificate -O "/etc/apt/sources.list" "https://raw.githubusercontent.com/jackzhang-superman/ocserv/main/us.sources.list"
			apt-get update
			apt-get install vim net-tools pkg-config build-essential libgnutls28-dev libwrap0-dev liblz4-dev libseccomp-dev libreadline-dev libnl-nf-3-dev libev-dev gnutls-bin -y
...
Set_tcp_port(){
	...
	read -e -p "(默认: 1600):" set_tcp_port
	[[ -z "$set_tcp_port" ]] && set_tcp_port="1600"
